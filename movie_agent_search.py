...
def main() -> int:
    ...
    config = load_config()
    title, year = parse_request(args.query)
    qb = config["qbittorrent"]
    client = QBittorrentClient(qb["host"], int(qb["port"]), qb["username"], qb["password"])

    client.login()
    search_id = client.start_search(args.query)
    ...
    print(f"Search results seen: {payload.get('total', len(results))}")
    print()
